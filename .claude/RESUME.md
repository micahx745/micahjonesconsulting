...
## Pass-57/58 (88bbafe) BUILT + previewed — AWAITING OPERATOR REVIEW, NOT pushed
Operator batch: hero face cut off · "Hire me" → weird page · trim stats to exits + $20M · nav
Clients/Contact scrolled mid-home · /work weak · case pages weak · review content for secrets.
Done: Nav = Services/Work/Playbook/About/Contact(/book), no fragment links (/about was orphaned);
hero photo top-anchored + 30% right overscan ≥861px, content dropped, TWO stats bottom-right;
/hire-me deleted → 308 /services (sitemap, citations, dead CSS); TitleCard dissolve scrub-driven
(was onEnter at scrollY 0 → empty first screen; motion-engineer fix); /work rebuilt on per-study
...
guardicore: "$150K deal size" + "Trillions in assets" unledgered → cut or ledger; "world's largest
public biomedical-research funder" is a one-answer riddle. All: em-dash cap broken 9-12×/file,
"roughly/~" hedges, numbers repeated 3×. Ordani RLS sentence overstates. Apply only after "go".

## Production state
Live = Pass-53..56c (deploy musrbwwz4, 2026-09-01, approval "push it"). Prod deploys are CLI
...
key, calendar ICS URL, Stripe test key; 3 envs each). 4. Stripe go-live. 5. Read the book.
