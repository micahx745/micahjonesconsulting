// Pass-128 screencast capture. EVIDENCE ONLY. Live site only.
//
// Reuses the exact mobile emulation, CPU throttle, and touch-swipe scroll
// mechanism from scroll-probe.mjs (see that file's header comment for why a
// manual touchStart/touchMove/touchEnd sequence is used instead of CDP
// Input.synthesizeScrollGesture -- that call is a measured no-op against
// this Chrome build). This script does NOT re-derive frame-interval or
// longtask numbers; it captures what actually painted, frame by frame, via
// CDP Page.startScreencast, so the operator's reported jank and world-switch
// "glitchiness" can be inspected visually rather than only statistically.
//
// Output (all under this pass-128 directory):
//   frames/frame-00001.png ...      -- every screencast frame, PNG
//   frames/manifest.json            -- per-frame {index, file, relMs, epochMs}
//   frames/world-switches.json      -- {relMs, epochMs, bg} per --cw-bg change
//   frames/run-summary.json         -- gesture count, scroll duration, etc.
//
// Usage: node screencast-scroll.mjs [--url URL] [--to SELECTOR] [--cpu N]
...
const OUT_DIR = path.join(process.cwd(), "frames");

function parseArgs(argv) {
  const options = { url: DEFAULT_URL, to: null, cpu: 4 };
...
async function installRecorders(page) {
  await page.evaluate(() => {
    ...
    // Track when reveal-gated headings/words cross into .is-in (ScrollReveal)
    // so the contact sheets can be cross-checked against a DOM-level signal,
    // not just eyeballed pixels.
    try {
...
async function main() {
  ...
  // Clear any stale frames from a prior run so the manifest always matches
  // what's on disk.
  ...
  try {
    ...
    // Frame capture bookkeeping. metadata.timestamp from CDP is seconds
    // since epoch on this machine's clock, same clock as Date.now() in the
    // page and in this Node process, so all three are directly comparable.
    let frameIndex = 0;
    const manifest = [];
    ...
    // Give the screencast a moment to start emitting before we begin
    // scrolling, so the pre-scroll resting frame is captured too.
    await sleep(300);
    ...
    // Let any in-flight frame events land before reading the manifest back.
    await sleep(200);
