...
declare -A DPL
for D in https://www.micahjonesconsulting.com https://micahjonesconsulting.vercel.app; do
  echo "== $D"
  ...
  n=$(printf '%s' "$PM" | grep -c 'never meant to be delivered'); chk "new postmates line" "$n" '[ "$n" -ge 1 ]' ">=1"
