#!/usr/bin/env bash
# card1-126.sh: card1-124.sh plus the Pass-126 How I work markers (the block marked "Pass-126" below).
# Built from card1-124, NOT card1-125: the full-time page is held (LESSONS #3 "FULL-TIME PAGE HELD"),
# so /full-time is neither checked nor expected. Run after the Pass-126 deploy:
#   EXPECT_DPL=<served id> bash .planning/exec/card1-126.sh
# Bite test before the push: 0 failures on a local `next start` of the gated build, and the Pass-126
# markers FAIL on the pre-push production.
# --- card1-124 header, kept as history ---
...
# Pass-126: read off the wire 2026-09-21 by the ship-check session, all three domains (www, the
# vercel.app alias, the apex) serving the same id: the Pass-124 home push (main = e8b44e3).
OLD_DPL="dpl_2mGqgwPQ1BwZCHpoi4wek2dupiwH"
...
for D in "${BASES[@]}"; do
  ...
  # Pass-126 (2026-09-21): How I work redesigned, same words, on / and /services (LESSONS #3 "HOW I WORK:
  # THE PLAN LINE" and "HOW I WORK REDESIGN TO THE SHIP CHECK"). Every string is ledgered. Markers stop
  # short of the no-break spaces; the NBSP itself is checked as bytes ($'\xc2\xa0') so this file stays ASCII.
  NB=$'\xc2\xa0'
  n=$(cnt "$HV" 'I give you a roadmap, and you approve it before development begins.'); chk "home Plan body, new" "$n" '[ "$n" -ge 1 ]' ">=1"
  n=$(cnt "$HV" 'A defined piece of the work ships within the first'); chk "home month-one line, new" "$n" '[ "$n" -ge 1 ]' ">=1"
  n=$(cnt "$HV" "within the first${NB}month."); chk "home NBSP before month." "$n" '[ "$n" -ge 1 ]' ">=1"
  n=$(cnt "$H" 'Something named ships'); chk "home old month-one line gone (raw)" "$n" '[ "$n" -eq 0 ]' 0
  n=$(cnt "$H" 'you sign off on it before I build'); chk "home old Plan body gone (raw)" "$n" '[ "$n" -eq 0 ]' 0
  n=$(cnt "$H" 'cw-hiw cw-hiw--home'); chk "home How I work component" "$n" '[ "$n" -ge 1 ]' ">=1"
  n=$(cnt "$css" 'cw-hiw__step'); chk "How I work CSS served" "$n" '[ "$n" -ge 1 ]' ">=1"
  n=$(cnt "$css" 'cw-principles--steps'); chk "old steps CSS gone" "$n" '[ "$n" -eq 0 ]' 0
  SV=$(vis "$D/services"); SR=$(raw "$D/services")
  n=$(cnt "$SV" 'How I work.'); chk "/services How I work heading" "$n" '[ "$n" -ge 1 ]' ">=1"
  n=$(cnt "$SR" 'cw-hiw cw-hiw--services'); chk "/services How I work component" "$n" '[ "$n" -ge 1 ]' ">=1"
  n=$(cnt "$SV" 'Week one is an audit and a scope.'); chk "/services Scope step" "$n" '[ "$n" -ge 1 ]' ">=1"
  n=$(cnt "$SV" 'I give you a roadmap, and you approve it before development begins.'); chk "/services Plan body" "$n" '[ "$n" -ge 1 ]' ">=1"
  n=$(cnt "$SV" "within the first${NB}month."); chk "/services NBSP before month." "$n" '[ "$n" -ge 1 ]' ">=1"
  n=$(cnt "$SV" 'I build the real thing, not a prototype.'); chk "/services Build step" "$n" '[ "$n" -ge 1 ]' ">=1"
  n=$(cnt "$SV" 'I stay for launch and what customers'); chk "/services Stay step" "$n" '[ "$n" -ge 1 ]' ">=1"
  n=$(cnt "$SV" 'No discovery fee. Any one of the three areas below, two of them, or all three.'); chk "/services note" "$n" '[ "$n" -ge 1 ]' ">=1"
  n=$(cnt "$SV" 'When the work is bigger than one person, I say so on the call.'); chk "/services Why one person kept" "$n" '[ "$n" -ge 1 ]' ">=1"
  n=$(cnt "$SR" 'Every engagement includes'); chk "/services old inclusions gone (raw)" "$n" '[ "$n" -eq 0 ]' 0
  n=$(cnt "$SR" 'Something named ships'); chk "/services old month-one line gone (raw)" "$n" '[ "$n" -eq 0 ]' 0
  n=$(cnt "$SR" 'Week one is the scoping session'); chk "/services old week-one sentence gone (raw)" "$n" '[ "$n" -eq 0 ]' 0
  n=$(cnt "$SR" 'Week one is a scoping session'); chk "/services old inclusions week-one gone (raw)" "$n" '[ "$n" -eq 0 ]' 0
done
if [ "$CHECK_DPL" -eq 1 ]; then
