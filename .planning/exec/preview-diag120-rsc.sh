...
# Lighthouse loop on the preview's /work with Next viewport prefetch (RSC ?_rsc=) requests BLOCKED
# (--blocked-url-patterns): if simulated LCP falls to production's ~2.71s, the prefetches cause the lab gap.
...
OUT=.planning/exec/lh120-vercel/preview-norsc; mkdir -p "$OUT"
rm -f "$JAR"
...
for i in 1 2 3; do
  node $LH "$PREVIEW/work" --extra-headers="$HDR" --blocked-url-patterns="*_rsc=*" --only-categories=performance --output=json --output-path="$OUT/work-$i.json" --chrome-path="$CH" --chrome-flags="--headless=new" --quiet; echo "preview-norsc run $i rc=$?"
done
rm -f "$JAR" "$HDR" "$SHARE_FILE"
node -e '
const fs=require("fs");
const v=[1,2,3].map(i=>{const j=JSON.parse(fs.readFileSync(`.planning/exec/lh120-vercel/preview-norsc/work-${i}.json`,"utf8"));const a=j.audits;const m=a.metrics.details.items[0];
 const vids=(a["network-requests"]?.details?.items??[]).filter(r=>/_rsc=/.test(r.url)).map(r=>`${r.url.replace(/.*\//,"")}:${r.statusCode}`);
...
v.forEach((x,i)=>console.log(`  preview-norsc run ${i+1}: simLCP ${Math.round(x.lcp)} obsLCP ${Math.round(x.obs)} simFCP ${Math.round(x.fcp)} el ${x.el} video requests ${JSON.stringify(x.vids)}`));
const med=[...v.map(x=>x.lcp)].sort((a,b)=>a-b)[1];console.log(`preview-norsc median simulated LCP ${med.toFixed(1)}ms (production 2710.8ms, preview with video 3310.7ms)`);'
