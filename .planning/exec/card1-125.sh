#!/usr/bin/env bash
# card1-125.sh: card1-124.sh plus the Pass-125 /full-time markers (card1-125-markers.sh). Run after the
# Pass-125 deploy: EXPECT_DPL=<served id> bash .planning/exec/card1-125.sh. Pre-push, run it against a local
# `next start` of the build as: bash .planning/exec/card1-125.sh http://localhost:3125 (no dpl checks).
# OLD_DPL below = the deploy both domains served at 2026-09-21 ~12:05 PDT, read off the wire (LESSONS #45).
...
for D in "${BASES[@]}"; do
  ...
  for p in / /full-time /work /work/guardicore /work/rfp-engine /work/ordani /work/content-engine /work/birth-worker /about /services /contact /llms.txt /sitemap.xml; do
  ...
  # Pass-125 (2026-09-21): the /full-time page, its three footer links and the /about line.
  . "$(dirname "$0")/card1-125-markers.sh"
