# Persona review — micahjonesconsulting

Run this in CLAUDE CODE, in the repo, on FABLE. Not in Cowork: LESSONS #1 in
docs/LESSONS_LEARNED.md records a Cowork review of this exact site that was
about half confabulated - it critiqued a portrait, a /work/akamai case study,
and /work-with-me and /contact pages that did not exist. A persona read is the
most fabrication-prone shape there is, because taste claims are hard to falsify.
The evidence rule below is what that lesson produced. It is not optional.
...
- `grep -oiF` under git-bash on this machine returns FALSE ZEROES on this tree.
  It reported "Tel Aviv" absent from a page that contains it. Verify page text
  with Python, decoding UTF-8 explicitly, not with grep -F.
...
## WHAT NOT TO REPORT (operator-held; reporting these is a false positive)
...
- the faint "Micah Jones / Oakland, CA" metadata straps were REMOVED on purpose
  on 2026-09-02. Do not propose restoring them.

## THE FACT LEDGER

Read docs/LESSONS_LEARNED.md #3 before writing a single proposal. Every number
on this site is operator-confirmed and dated. NEVER propose copy that invents a
...
`.planning/reviews/PERSONA-REVIEW-<date>.md`
